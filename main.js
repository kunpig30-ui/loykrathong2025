/* ===== ปรับง่าย ===== */
const WATER_FACTOR = 0.80;   // ผิวน้ำเป็นสัดส่วนความสูงจอ
const ROAD_DY      = 0;      // เส้นแดง = ขอบน้ำพอดี
const LANES        = 5;
const LANE_STEP    = 16;
const KR_SIZE      = 60;
const MAX_BOATS    = 20;
...
class Krathong{
  ...
  update(dt){
    ...
    if(this.x>cvs.width+160) this.x=-160 - rnd(0,120); // วนซ้ายแบบเหลื่อม
  }
  draw(g){
...
/* เลือก “ใบที่ซ้ายสุด” เพื่อผูกข้อความ (จะออกก่อน) */
...
document.getElementById("launch")?.addEventListener("click", async ()=>{
  ...
  // เติมใบใหม่ได้เรื่อยๆ แต่ไม่เกิน 20
...
class Firework{
  ...
  draw(g){
    ...
    for(const p of this.parts){ const R=56*p.a; g.save(); g.globalAlpha=p.a; if(logoImg&&logoImg._ok) g.drawImage(logoImg,p.x-R,p.y-R,R*2,R*2); else { g.fillStyle="#fff"; g.beginPath(); g.arc(p.x,p.y,R,0,Math.PI*2); g.fill(); } g.restore(); }
  }
}
const fireworks=[];
...
function drawTuk(dt){
  ...
  const y=Math.max(waterY()+1, roadY()-tuk.h-2); // “บนเส้น” ไม่ตกน้ำ
...
function drawWater(){
  ...
  for(let i=0;i<10;i++){ ctx.beginPath(); for(let x=0;x<cvs.width;x+=20){ const yy=y+Math.sin((x/40)+waveT*1.2+i)*6+i*16; if(x===0) ctx.moveTo(x,yy); else ctx.lineTo(x,yy); } ctx.stroke(); }
}
