/* ===== Core tune ===== */
const WATER_FACTOR = (innerWidth < 480 ? 0.74 : 0.76); // ผิวน้ำสูงขึ้นนิด เพราะย้ายสถิติลงล่าง
const ROAD_DY = 2;       // เส้นแดงอยู่ชิดผิวน้ำ
const LANES = 5;
const LANE_STEP = 16;
const KR_SIZE = 60;
const MAX_BOATS = 20;
...
function renderWish(){
  ...
  document.getElementById('wishList').innerHTML =
    q.slice(-6).map(x=>`<li>• คนที่ ${x.n} 🕯️ ${x.w}</li>`).join('');
}
renderWish();
...
class Krathong{
  ...
  update(dt){ this.t+=dt; this.x+=this.vx*dt; if(this.x>cvs.width+160) this.x=-160-rnd(0,120); }
  draw(g){
...
/* เลือกใบที่ “ซ้ายสุด” เพื่อให้คำอธิษฐานไปกับใบที่จะออกก่อนเสมอ */
...
const fireworks=[]; function spawnTriple(){ const w=cvs.width; [w*.22,w*.5,w*.78].forEach(x=>fireworks.push(new Firework(x))); }
...
function drawTuk(dt){
  ...
  const y=roadY()-tuk.h-2;               // อยู่บนเส้นแดง
