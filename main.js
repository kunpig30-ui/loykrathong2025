...
(function(){
  ...
  // === Tune parameters ===
  const BG_SHIFT_PX = -12;   // move background up
  const ROAD_DY     = 18;    // red road line below water
  const LANES       = 5;     // krathong lanes
  const LANE_STEP   = 14;
  const BUBBLE_PADY = 0.9;
  const KR_SIZE     = 60;

  // Move background if exists
  ...
  const makeImg = p => { 
    ...
    i.decoding='async';
    i.onload=()=>i._ok=true;
    i.onerror=()=>console.warn('image not found:', p);
    i.src=p+V;
    return i;
  };
  ...
  class Krathong{
    ...
    draw(g){
      ...
      if(this.text){
        const msg = this.text;
        g.save(); g.font=(innerWidth<=420?"600 14px":"600 16px")+" system-ui,'TH Sarabun New',Prompt,sans-serif";
        ...
        g.beginPath(); g.roundRect(cx-w/2,cy-h/2,w,h,14); g.fill();
        g.globalAlpha=1; g.strokeStyle="rgba(255,255,255,.28)"; g.lineWidth=1;
        g.beginPath(); g.roundRect(cx-w/2,cy-h/2,w,h,14); g.stroke();
  ...
  function drawWater(){
    ...
    for(let i=0;i<10;i++){ ctx.beginPath();
      for(let x=0;x<cvs.width;x+=20){ const yy=y+Math.sin((x/40)+waveT*1.2+i)*6+i*16;
