/* v6 — responsive anchors from background, tuk on red line, 5 lanes no collision,
        wish bubble sticks with boat till right edge, mobile sizes tuned */
(function () {
  const cvs = document.getElementById('scene');
  const ctx = cvs.getContext('2d');
  const header = document.querySelector('header');
  const bgEl = document.getElementById('bgLayer'); // << ใช้ยึดตำแหน่งจากภาพพื้นหลัง
  ...
  // ---------- anchor by background image ----------
  function mobileAnchors() {
    // ปรับตามอัตราส่วนจอมือถือ (ทดลองแล้วคม)
    const ar = innerHeight / Math.max(1, innerWidth);
    if (ar >= 1.95) return { waterPct: 82, roadPct: 96, kr: 64 };   // จอยาวมาก
    if (ar >= 1.35) return { waterPct: 80, roadPct: 95, kr: 64 };   // ส่วนใหญ่
    return { waterPct: 78, roadPct: 94, kr: 65 };                   // จอกว้าง
  }
  function anchoredYFromBg(pct) {
    if (!bgEl || !bgEl.complete) return Math.round(cvs.height * (pct / 100));
  ...
  function waterY() {
    if (IS_MOBILE) return anchoredYFromBg(mobileAnchors().waterPct);
    // desktop: ใช้ 80% ของพื้นที่หลัง header
    return Math.round(cvs.height * 0.80);
  }
  // กำหนดเส้นถนน “ตรงกับเส้นแดงในภาพพื้นหลัง”
  // ถ้าอยากเลื่อนขึ้น/ลงละเอียด ใช้ ROAD_DY (+ ขึ้น, - ลง)
  const ROAD_DY = 0; // ปรับละเอียด 1–3 px ตามสายตา
  function roadY() {
    if (IS_MOBILE) return anchoredYFromBg(mobileAnchors().roadPct) + ROAD_DY;
    return anchoredYFromBg(95) + ROAD_DY;
  }
  ...
  // ---------- krathong: 5 lanes, no collision ----------
  const LANES = 5;
  const STEP = 14;
  function krSize() {
    return IS_MOBILE ? mobileAnchors().kr : 60;
  }
  function laneY(i) {
    // ให้สูงกว่าขอบล่างเล็กน้อย ป้องกัน “ติดขอบล่าง”
    const base = waterY() - krSize() * 0.5 + 48;
    return Math.min(cvs.height - 20, base + i * STEP);
  }
  ...
  class Krathong {
    constructor(img, lane, offset) {
      this.img = img;
      this.lane = lane;
      this.x = -220 - offset;                  // เยื้องกันตอนเริ่ม
      this.vx = rnd(22, 28);
    ...
    update(dt) {
      this.t += dt;
      this.x += this.vx * dt;
      if (this.x > cvs.width + 160) this.x = -160; // วนซ้ายใหม่
    }
    draw(g) {
      const wy = waterY();
      // shadow
      ...
      if (this.img && this.img._ok) {
        g.drawImage(this.img, this.x - krSize() / 2, this.y - krSize() / 2, krSize(), krSize());
      } else {
        g.fillStyle = '#27ae60'; g.beginPath(); g.arc(this.x, this.y, krSize() / 2, 0, Math.PI * 2); g.fill();
      }
      // wish — อยู่จนลากถึงขอบขวา (ไม่นับเวลา)
      if (this.text) {
  ...
  class Firework {
    ...
    explode() {
      const N = 12;
      for (let i = 0; i < N; i++) {
        const a = i / N * Math.PI * 2, sp = 100 + rnd(0, 50);
  ...
  function drawTuk(dt) {
    ...
    let y = roadY() - tuk.h;
    // กันตกน้ำ
    y = Math.max(waterY() + 6, Math.min(y, cvs.height - tuk.h));
